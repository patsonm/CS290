...
function loadTable() {
    $.ajax({
        type: 'GET',
        url: "http://flip2.engr.oregonstate.edu:7234/get-workouts",
 ...
 $(document).ready(function() {
      if (window.location.pathname == '/edit-row') {
        ...
        $.ajax({
            type: 'GET',
            url: "http://flip2.engr.oregonstate.edu:7234/get-a-workout?id=" + id,
...
function submit () {
    ...
    $.ajax({
        type: 'GET',
        url: "http://flip2.engr.oregonstate.edu:7234/submit?name=" + name + "&weight=" + weight + "&weightUnit=" + weightUnit + "&reps=" + reps + "&date=" + date,
        success: function (id) {
            data.id = id;
            drawRow(data);
        }
...
function save() {
    ...
    $.ajax({
        type: 'GET',
        url: "http://flip2.engr.oregonstate.edu:7234/edit-save?id=" + id + "&name=" + name + "&weight=" + weight + "&lbs=" + lbs + "&reps=" + reps + "&date=" + date,
        success: function () {
            window.location = "http://flip2.engr.oregonstate.edu:9900/";
        }
    });
}
...
function edit(id) {
    $.ajax({
        ...
        success: function (data) {
            var rowData = JSON.parse(data)[0];            
            window.location = "http://flip2.engr.oregonstate.edu:7234/edit-row?id=" + rowData.id;
...
function deleteRow(id) {
    $.ajax({
        type: 'GET',
        url: "http://flip2.engr.oregonstate.edu:7234/delete?id=" + id,
        success: function () {
            $("#" + id).remove();
        }
    });
